...
export const IngestionOverlay = ({ filename, estimatedSeconds }: IngestionOverlayProps) => {
  ...
  // 2. The Browser Notification Request
  ...
  return (
    <div className="absolute inset-0 bg-zinc-950/95 backdrop-blur-md z-50 flex items-center justify-center p-4">
      <div className="flex flex-col items-center max-w-md w-full">
        ...
        {/* SOTA: The "Axiom Auditor" Animation (SVG + Framer Motion) */}
        ...
        <p className="text-zinc-400 text-sm text-center mb-6">
          ...
          Large documents may take a moment to vectorize.
        </p>
        ...
        <div className="bg-zinc-900 border border-white/5 rounded-2xl p-6 w-full flex flex-col items-center shadow-2xl">
          ...
          <button 
            ...
            className={cn(
              ...
              notifyGranted 
                ...
                : "bg-white/5 text-white hover:bg-brand-primary hover:text-black border border-white/10"
            )}
          >
            {notifyGranted ? (
              <><BellRing size={16} className="animate-bounce" /> Alerts Activated</>
            ) : (
          ...
          {!notifyGranted && (
             <p className="text-zinc-600 text-[10px] mt-3 text-center">
               Enable notifications so you can switch tabs or minimize the browser while we work.
             </p>
          )}
        </div>
      </div>
