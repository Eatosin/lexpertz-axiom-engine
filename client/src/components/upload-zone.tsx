...
export const UploadZone = ({ onUploadComplete }: UploadZoneProps) => {
  ...
  const validateAndSetFile = (selectedFile: File) => {
    ...
    // Cap at 50MB (Enterprise Standard)
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setUploading(false); // Only unlock if it fails
    } 
  };
          ...
          <motion.div
            ...
            className={cn(
              ...
              isDragging 
                ...
                : "border-white/10 hover:border-brand-primary/40 bg-white/5"
            )}
          >
            <input 
              type="file" 
            ...
            <div className={cn(
              ...
              isDragging 
                ? "bg-brand-primary text-black scale-110 shadow-[0_0_20px_rgba(16,185,129,0.5)]" 
                : "bg-white/5 border border-white/10 text-zinc-500 group-hover:text-brand-primary group-hover:bg-brand-primary/10 group-hover:scale-110"
            )}>
              <UploadCloud size={32} />
            </div>
            <h3 className="text-lg font-bold text-white mb-2 relative z-10">
              {isDragging ? "Drop to Secure Vault" : "Upload Evidence Document"}
            </h3>
              ...
              <div className="flex items-center gap-4 overflow-hidden">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-zinc-500 text-xs font-mono uppercase tracking-wider">
                    {uploading ? "Transmitting Bytes..." : "Ready for Ingestion"}
                  </p>
                </div>
              </div>
              {!uploading && (
