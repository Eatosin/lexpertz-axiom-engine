...
export function PricingVault() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simulate network delay for the "Encryption" feel
    // We will wire this to Supabase in Step 2!
    setTimeout(() => {
      setStatus("success");
      setEmail("");
    }, 1500);
  ...
  // Fake pricing data just for the blurred background effect
  const dummyTiers =[
            ...
            <AnimatePresence mode="wait">
              {status === "success" ? (
                <motion.div 
                  ...
                  className="bg-brand-primary/10 border border-brand-primary/30 rounded-2xl p-6 text-center space-y-3"
                >
                  ...
                  <p className="text-zinc-400 text-xs">Your cryptographic signature (email) has been securely logged. We will contact you shortly.</p>
                </motion.div>
              ) : (
                <motion.form 
                  key="form"
                ...
                >
                  <div className="relative group">
                    ...
                    <input 
                      ...
                      className="w-full bg-[#111] border border-zinc-800 focus:border-brand-primary/50 rounded-xl py-4 pl-12 pr-4 text-white text-sm outline-none transition-all placeholder:text-zinc-600 shadow-inner disabled:opacity-50"
                    />
                  </div>
                  <button 
                    type="submit"
