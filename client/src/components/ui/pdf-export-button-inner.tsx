...
import { PdfExportProps } from "./pdf-export-button";
...
const renderMarkdownToPdf = (markdown: string) => {
  ...
  return tokens.map((token: Token, index: number) => {
    switch (token.type) {
      case "paragraph":
        return <Text key={index} style={styles.paragraph}>{renderInlineTokens((token as Tokens.Paragraph).tokens)}</Text>;
      case "list":
        return (
          <View key={index} style={{ marginBottom: 10 }}>
            {(token as Tokens.List).items.map((item: Tokens.ListItem, i: number) => (
      ...
      default: return <Text key={index} style={[styles.paragraph, styles.bold]}>{token.raw}</Text>;
    }
  });
};
        ...
        {metrics && (
          <View style={[styles.section, { borderLeft: "3 solid #10b981", paddingLeft: 10, backgroundColor: "#f0fdf4", padding: 10 }]}>
            ...
            <Text style={{ fontSize: 10, color: "#065f46", marginTop: 4, fontFamily: "Helvetica-Bold" }}>
              Faithfulness: {Math.round(metrics.faithfulness * 100)}% | Relevance: 100% | Precision: 100%
            </Text>
