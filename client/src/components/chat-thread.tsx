...
import { ShieldCheck, User, CheckCircle2, ArrowDown, Sparkles } from "lucide-react";
...
// SOTA: Dynamic text mapping based on LangGraph active node
const getThinkingText = (step?: number) => {
  switch (step) {
    case 0: return "Scanning Sovereign Vault...";
    case 1: return "Synthesizing Evidence (Editor Node)...";
    case 2: return "Architecting Report (Reasoning)...";
    case 3: return "Verifying Mathematical Faithfulness (Prosecutor)...";
    default: return "Initializing Neural Link...";
  }
};

// SOTA: The Claude / Google AI Studio "Thinking" Indicator
  ...
  return (
    ...
    >
      <div className="relative flex items-center justify-center">
        ...
        />
        <Sparkles size={16} className="text-brand-secondary relative z-10 animate-pulse" />
      </div>
      
      {/* The Dynamic Text with a morphing animation when it changes */}
      <AnimatePresence mode="wait">
        <motion.span
          ...
          className="text-sm font-medium bg-gradient-to-r from-zinc-300 to-zinc-500 bg-clip-text text-transparent italic"
        >
          {text}
        </motion.span>
      </AnimatePresence>
...
const AssistantMessage = memo(({ m, userQuery, activeContext }: { m: Message, userQuery: string, activeContext: string }) => {
  ...
  return (
    <div className="w-full flex flex-col space-y-4 pt-1">
      ...
      {/* 1. THE NEW "THINKING" UX (Google AI Studio Style) */}
      <AnimatePresence>
        {m.status === "reasoning" && (
      ...
      {/* 3. VERIFICATION FOOTER (Appears smoothly when done) */}
      <AnimatePresence>
        {m.status === "verified" && (
          <motion.div 
          ...
          >
            ...
            {/* SOTA: Minimalist Seal of Verification */}
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-full text-[10px] font-mono text-emerald-500 uppercase font-bold tracking-tighter">
                <CheckCircle2 size={12} /> Gated Logic Verified
              </div>
            </div>
          </motion.div>
        )}
