...
async def test_sse_retry_clear_signal(mock_astream_events):
    ...
    async def mock_generator(*args, **kwargs):
        ...
        yield {"event": "node_update", "data": {"node": "Architect", "status": "active"}}
        yield {"event": "token", "data": {"text": "BAD DRAFT TEXT."}}
        
        # Prosecutor grades low → triggers retry
        yield {"event": "node_update", "data": {"node": "Prosecutor", "status": "active"}}
        yield {"event": "on_chain_end", "name": "grade_generation_node", "data": {"output": {"metrics": {"faithfulness": 0.0}}}}
        
        # CLEAR SIGNAL (this is what the UI listens for)
        yield {"event": "clear", "data": {"message": "retry_triggered"}}
        
        # Architect retry (good draft)
        yield {"event": "node_update", "data": {"node": "Architect", "status": "active"}}
        yield {"event": "token", "data": {"text": "GOOD DRAFT TEXT."}}
