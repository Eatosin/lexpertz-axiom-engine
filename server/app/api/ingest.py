...
async def process_document(file_path: str, filename: str, user_id: str) -> None:
    ...
    """
    try:
        # 1. Extraction (Unstructured logic)
        ...
        if db:
            ...
            # Explicitly cast to List[dict] to satisfy MyPy indexing
        ...
        # 3. Processing Loop (Vectorization)
        ...
        # 4. Batch Commit to Supabase
        if db:
            # Bypass Supabase Union type check with Any cast
...
async def ingest_document(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    """
    Secure Entry Point for Document Ingestion.
    """
